...
// I2C LCD의 기본 주소
// [수정] LCD를 2개 쓰기 위해 주소를 2개로 분리
#define LCD1_ADDR 0x4E // 기존 LCD (0x27 << 1)
#define LCD2_ADDR 0x4C // 새 LCD   (0x26 << 1)
...
void I2C1_Write_Byte_To(unsigned char addr, char data) {
    ...
    I2C1->DR = addr;               // [수정] LCD_ADDR 대신 인자로 받은 addr 전송
...
// I2C 데이터 전송 기본 함수
// [유지] 기존 코드와의 호환을 위해 남겨둠 -> 기본적으로 LCD1로 전송
...
// [유지] 기존 함수 -> 기본적으로 LCD1에 전송
void LCD_Send(char data, int mode) {
    LCD_Send_To(LCD1_ADDR, data, mode);
}

void LCD_Cmd(char cmd) { LCD_Send_To(LCD1_ADDR, cmd, 0x00); }    // [수정] 기본 LCD1로 명령 전송
void LCD_Data(char data) { LCD_Send_To(LCD1_ADDR, data, 0x01); } // [수정] 기본 LCD1로 데이터 전송
...
void LCD_Init(void) {
    I2C1_Init();
    LCD_Init_To(LCD1_ADDR); // [수정] 기존 LCD만 초기화
}
